...
public class HttpPrioritizedServerTest {
    ...
    public void testGetHistoryTasks() throws IOException, InterruptedException {
        ...
        URI urlPrioritized = URI.create("http://localHost:8080/api/prioritized");

        HttpRequest requestHistory = HttpRequest.newBuilder()
                .uri(urlPrioritized)
                .GET()
                .build();
        HttpResponse<String> responsePrioritized = httpClient.send(requestHistory, HttpResponse.BodyHandlers
                .ofString());

        List<Task> tasksFromServerPrioritized = gson.fromJson(responsePrioritized.body(), taskListType);

        assertEquals(tasksFromServerPrioritized.getFirst().getStartTime().format(formatter), LocalDateTime.now()
                        .format(formatter),
                "Время задачи не выстроено правильно");
        assertEquals(tasksFromServerPrioritized.get(1).getStartTime().format(formatter), LocalDateTime.now()
                        .plusDays(1).format(formatter),
                "Время задачи не выстроено правильно");
        assertEquals(tasksFromServerPrioritized.get(2).getStartTime().format(formatter), LocalDateTime.now().plusDays(2)
                        .format(formatter),
                "Время задачи не выстроено правильно");
    }
}